		...
		if (not teleportedServers) and (not shared.VapeIndependent) then
			...
			local teleportScript = [[
				...
				if shared.VapeDeveloper then
					...
					loadstring(game:HttpGet('https://raw.githubusercontent.com/0x6767/VapeV67ForRoblox/'..readfile('vape67/profiles/commit.txt')..'/loader.lua', true), 'loader')()
				end
			]]
			if shared.VapeDeveloper then
...
if not shared.VapeIndependent then
	...
	if isfile('vape67/games/'..game.PlaceId..'.lua') then
		...
		if not shared.VapeDeveloper then
			local suc, res = pcall(function()
				return game:HttpGet('https://raw.githubusercontent.com/0x6767/VapeV67ForRoblox/'..readfile('vape67/profiles/commit.txt')..'/games/'..game.PlaceId..'.lua', true)
			end)
